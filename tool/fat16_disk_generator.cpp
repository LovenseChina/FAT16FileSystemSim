...
public:
    ...
    //static constexpr int32_t BS_VOl_ID = 0;     //  卷的序列号，使用随机数生成
...
/********** 成员函数定义 **********/
FAT16DiskGenerator::FAT16DiskGenerator(const std::string &disk_name) : fat_table(8192, 0),
...
void FAT16DiskGenerator::gen_ds_data()
{
    ...
    this->first_sector._BS_END_.BS_VolID = std::rand(); //  卷序列号采用随机值
    ...
    //  根目录初始化
    std::for_each(this->root_entry_table.begin(), this->root_entry_table.end(), [](DIR_ENTRY &ent)
                  { memset(&ent, 0, sizeof(ent)); });   // 全部根目录置0
    /**
     * @brief 卷标文件设置
     * - 需要保持与 BS_VolLab 指同步
     * - 具体设置见代码
     */
    this->root_entry_table[0].DIR_Attr = 0x08;
    memset(this->root_entry_table[0].DIR_Name, ' ', 11);
    memcpy(this->root_entry_table[0].DIR_Name, "TJC vdisk", 9);
}
