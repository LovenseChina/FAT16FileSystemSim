...
struct PathResult
{
    ...
    std::string file_name;        // 文件名的 8.3 格式
 ...
 * 
 * 行为说明：
 * - "/" → 根目录，parent_dir_cluster = 0xFFFF, file_name = ""
 ...
 */
bool resolve_path(const std::string &path, PathResult &result);
...
/********** FAT16文件系统数据成员 **********/
    ...
    std::string pwd;    //  当前目录
    std::vector<DIR_ENTRY> sub_dir_file;    //  子目录，按簇加载使用,因为子目录也是“普通文件”按簇链管理
